      ...
      if ( strcmp( cfg.method, 'opdPE' ) == 1 )
        if ( isfield( cfg, 'time' ) )
          if ( isfield( cfg, 'units' ) )
              clear outdata %% debug
              if strcmp(cfg.method, 'opdPE')
                  [ ePE, ordPatPlot, ordDistPlot ]  = fn( indata( iChannel, : ), cfg.delay, cfg.order, cfg.orderSeq, cfg.windowSize, cfg.time, cfg.units );
                  outdata.ePe = ePE;
                  outdata.ordPatPlot = ordPatPlot;
                  outdata.ordDistPlot = ordDistPlot;
              else
...
if ( strcmp( cfg.method, 'opdPE' ) ~= 1 && ...
  ...
  for iChannel = 1:nChannels
    ...
    if ( isfield( cfg, 'time' ) )
      if ( numel( cfg.time ) == length( indata ) )
        plot( cfg.time, indata( iChannel, : ), 'k', 'LineWidth', lineWidth ); grid on;%% first plott PE
        if ( isfield( cfg, 'units' ) )
          xlabel( cfg.units );
        end
        if ( nChannels > 1 )
        ...
        plot( cfg.time( end - length( outdata( iChannel, : ) ) + 1:end ), ...
              outdata( iChannel, : ), 'k', 'LineWidth', lineWidth ); grid on; %% second plot PE
        linkaxes( [ ax1, ax2 ], 'x' );
      else
