function burst_res = burst_analysis1(app)
%BURST_ANALYSIS this is the not ready updated version of burstanalysis when
%finished it needs to replace burst analysis
...
thresh = app.edt_threshold.Value;
window = app.edt_window.Value;
...
if app.chkbx_manual.Value 
    ...
    HThreshold = basmed+ thresh*basStd; 
end
...
if app.chkbx_th_on_silent.Value
    ...
    sil_med = median(x2);
    sil_Std = std(x2);
    HThresholdOutlier = sil_med +app.edt_outlier_threshold.Value *thresh *sil_Std;
    HThreshold=  sil_med+ thresh*sil_Std;
    clear x2
    
else
    HThresholdOutlier = basmed +app.edt_outlier_threshold.Value *thresh *basStd;    %% outlier threshold     
end
