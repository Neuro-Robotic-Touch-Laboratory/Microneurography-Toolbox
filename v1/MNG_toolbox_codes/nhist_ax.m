...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [theText,rawN, x] = nhist_ax(cellValues, varargin)
...
%           you need to clear the whole figure to use sub-plot
            ...
            cla(ax); %just in case we have some subploting going on you don't want to ruin that
            axis(ax, 'normal');
            legend(ax,'off'); % in case there was a legend up.
...
% But we might as well run it to set the fontsize here:
    for k=1:num2Plot
        if num2Plot>1
            subplot(num2Plot,1,k,ax);
        end
        hold(ax,'on');
        plot(ax,[.5 1.5],[-1 -1],'color',lineStyleOrder{k},'linewidth',linewidth);
...
%           doesn't allow changing the lineColor property of a histc bar graph.)
            if vertLinesFlag % then plot the bars with edges
                bar(ax,logFunc(x{k}+binWidth(k)/2),n{k}/1,'FaceColor',faceStyleOrder{k},'barwidth',1,'EdgeColor','k','linewidth',1.5)
            else % plot the bars without edges
                bar(ax,logFunc(x{k}+binWidth(k)/2),n{k}/1,'FaceColor',faceStyleOrder{k},'barwidth',1,'EdgeColor','none')
            end
            
            if ~smoothFlag
                stairs(ax,logFunc(x{k}),n{k},'k','linewidth',linewidth);
                plot(ax,logFunc([x{k}(1) x{k}(1)]),[0 n{k}(1)],'color','k','linewidth',linewidth);
...
%                 not to (end-1) like above, so we got an extra digit
                ...
                plot(ax,logFunc(xi),yi,'k','linewidth',linewidth);
            end
    ...
    for k=1:num2Plot
        if isData(k)
            if ~smoothFlag
                stairs(ax,logFunc(x{k}),n{k},'color',lineStyleOrder{k},'linewidth',linewidth);
                plot(ax,logFunc([x{k}(1) x{k}(1)]),[0 n{k}(1)],'color',lineStyleOrder{k},'linewidth',linewidth);
            else % plot it smooth
...
%                 not to (end-1) like above, so we got an extra digit
                yi = pchip([x{k}(1)-binWidth(k)/2, x{k}(1:end)+binWidth(k)/2],[0 n{k}(1:end-1) 0],xi);
%                 xi = [linspace(SXRange(1),SXRange(2),500)]; yi = pchip(x{k}(1:end-1)+binWidth(k)/2,n{k}(1:end-1),xi);
                plot(ax,logFunc(xi),yi,'color',lineStyleOrder{k},'linewidth',linewidth);
                if vertLinesFlag % plot those points, otherwise its a wash.
                    plot(ax,logFunc(x{k}(1:end-1)+binWidth(k)/2),n{k}(1:end-1),'.','color',lineStyleOrder{k},'markersize',15);
                end
            end
        end
    end
...
if normalHist
    for k=1:num2Plot
        ...
        if cropped_right{k} % if some of the data points lie outside the bins.
            text(ax,logFunc(x{k}(end-1)+binWidth(k)/10),n{k}(end-1)+max(n{k})/50,'*','fontsize',AxisFontSize,'color',lineStyleOrder{k});
        end
        if cropped_left{k} % if some of the data points lie outside the bins.
            text(ax,logFunc(x{k}(1)+binWidth(k)/10),n{k}(1)+max(n{k})/30-max(n{k})/50,'*','fontsize',AxisFontSize,'color',lineStyleOrder{k});
        end
        end
    end
else
...
%             plot([x{k}(1) x{k}(1)],[0 n{k}(1)],'color',lineStyleOrder{k},'linewidth',linewidth);
      ...
      %    ADD A STAR IF CROPPED 
            if cropped_right{k} % if some of the data points lie outside the bins.
                text(ax,logFunc(x{k}(end-1)+binWidth(k)/10),n{k}(end-1)+maxN/50,'*','fontsize',AxisFontSize,'color',lineStyleOrder{k});
            end
            if cropped_left{k} % if some of the data points lie outside the bins.
                text(ax,logFunc(x{k}(1)+binWidth(k)/10),n{k}(1)+maxN/30-maxN/50,'*','fontsize',AxisFontSize,'color',lineStyleOrder{k});
            end
        end
    end
end
...
%       roundedMode{k}=mean(x{k}(n{k}==max(n{k}))); % finds the maximum of the plot
    ...
    if medianFlag
        if normalHist % plot with 'MarkerFaceColor'
            stem(ax,logFunc(medianV{k}),(1.1)*tempMax,'color',lineStyleOrder{k},'linewidth',linewidth,'MarkerFaceColor',faceStyleOrder{k});
        else % plot hollow
            stem(ax,logFunc(medianV{k}),(1.1)*tempMax,'color',lineStyleOrder{k},'linewidth',linewidth)
        end
...
%               Note that this mode is rounded . . .
        if medianFlag % plot the mode in a different way
            if normalHist
                stem(ax,logFunc(roundedMode{k}),(1.1)*tempMax,'--','color',lineStyleOrder{k},'linewidth',linewidth,'MarkerFaceColor',faceStyleOrder{k});
            else
                stem(ax,logFunc(roundedMode{k}),(1.1)*tempMax,'--','color',lineStyleOrder{k},'linewidth',linewidth)
            end
...
%           note: that it is plotting it from the top to the bottom! just like the legend is
        ...
        if normalHist % plot only the dot in color
            plot(ax,logFunc(meanV{k}),tempY,'.','markersize',25,'color',faceStyleOrder{k});
            plot(ax,logFunc(meanV{k}),tempY,'o','markersize',8,'color',[0 0 0],'linewidth',linewidth);
        else % plot everything in color, this dot and the bars before it
            plot(ax,logFunc(meanV{k}),tempY,'.','markersize',25,'color',lineStyleOrder{k});
        end
    end
%   Plot the boxplot!
    if boxplotFlag % it is important to set the ylim property here so errorb plots the appropriate sized error bars
        ...
        if normalHist % plot the thing with all the colors, there is just one of them.
            bplot(logFunc(cellValues{k}),ax,tempY,'barwidth',boxplotWidth,'linewidth',linewidth,'horizontal',pointsOrNo,'specialwidth','histmode');
        else % plot with the color for that data set
            bplot(logFunc(cellValues{k}),ax,tempY,'color',lineStyleOrder{k},'barwidth',boxplotWidth,'linewidth',linewidth,'horizontal',pointsOrNo,'specialwidth','histmode');
        end
...
hold(ax, 'on'); % axis is already cleared if hold was off
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%
function forLegend = bplot(x,ax,varargin)
%% save the initial hold state of the figure.
hold_state = ishold(ax);
if ~hold_state
    clf;
...
if size(x,1)>1 && size(x,2)>1 % great, you want to plot a bunch.
    if isempty(varargin)
        forLegend = bplot(x(:,1),ax,1);
        for ii=2:size(x,2)
            hold(ax, 'on');
            bplot(x(:,ii),ax,ii,'nolegend');
        end
    else
        if ~ischar(varargin{1})
            warning('You can''t specify a location for multiple guys, this will probably crash');
        end
        forLegend = bplot(x(:,1),ax,1,varargin{:});
        for ii=2:size(x,2)
            hold(ax, 'on');
            bplot(x(:,ii),ax,ii,'nolegend',varargin{:});
...
if horizontalFlag
    hReg2(end+1) = rectangle(ax,'Position',[boxEdge(1),y-barWidth/2,IQR,barWidth],'linewidth',linewidth,'EdgeColor',boxColor);

    hold(ax, 'on');
    hReg2(end+1) = plot(ax,[medianX medianX],[y-barWidth/2 y+barWidth/2],'color',meanColor,'linewidth',linewidth);
    if meanFlag
        hReg2(end+1) = plot(ax,meanX,y,'+','color',meanColor,'linewidth',linewidth,'markersize',10);
    end
    ...
    hReg2(end+1) = rectangle(ax,'Position',[y-barWidth/2,boxEdge(1),barWidth,IQR],'linewidth',linewidth,'EdgeColor',boxColor);
    hold(ax, 'on');
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(yy)
        ...
        if ~isempty(xx)
            if horizontalFlag
                hReg2(6) = plot(ax,xx,yy,'o','linewidth',linewidth,'color',wisColor);
            else
                 hReg2(6) = plot(ax,yy,xx,'o','linewidth',linewidth,'color',wisColor);
            end
        end
    end
end
