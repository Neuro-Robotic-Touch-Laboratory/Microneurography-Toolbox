function [clu, tree] = run_cluster_loc(par, multi_files)
dim = par.inputs;
fname = par.fnamespc;
...
switch system_type
    ...
    case {'PCWIN64'}    
        ...
        [status,result] = dos(sprintf('"%s" %s.run',which('cluster_64.exe'),fname));%fname(tmp+1:end
...
delete([fname '*.edges']);
delete([fname '*.param']);
delete([fname '.dg_01.lab'])%% Test
delete([fname '.dg_01'])%% Test
end
