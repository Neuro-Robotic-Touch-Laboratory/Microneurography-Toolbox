...
spike_res_out.use_spikes(1:length(spike_res_in.spike_idx),1) = ones(size(spike_res_in.spike_idx));
for i = 1 : length(burst_ints)
    spike_res_out.use_spikes(1:length(spike_res_in.spike_idx),i+1) = zeros(size(spike_res_in.spike_idx));
    for j = 1:size(burst_ints(i).borders,1)
        ts_idx =  find((spike_res_in.spike_ts/1000) >= burst_ints(i).borders(j,1) & (spike_res_in.spike_ts/1000) <= burst_ints(i).borders(j,2));
        spike_res_out.use_spikes(ts_idx,i+1) = 1;
    end
end
