...
if ~isempty(f_names{1})
    ...
    for i =1:length(tmp_names)
        ...
        for j = 1:length(tmp_ints)
            ...
            figs(i).ints(j).anas = struct('burst', struct('done', false, ...
                                                          'amp', struct('format', [false,false,false]),...
                                                          'dur', struct('format', [false,false,false]),...
                                                          'int', struct('format', [false,false,false]),...
                                                          'rr',  struct('format', [false,false,false])),...
                                          'spike', struct('done', false,...
                                                          'subint', struct('name',[],...
                                                                           'format', [false,false,false],...
                                                                           'unit',struct('name',[], ...
                                                                                         'format', [false,false,false]))),... 
                                          'spikerate', struct('done', false,... 
                                                               'unit',struct('name',[],...
                                                                             'format',[false,false,false])),...
                                          'spectral', struct('done', false,...
                                                             'subint', struct('name', [],...
                                                                              'signal', struct('name', [],...
                                                                              'format', [false,false,false]))),...
                                          'wavelet', struct('done', false, ...
                                                            'subint', struct('name', [],...
                                                                             'signal', struct('name', [],...
                                                                                              'format', [false,false,false]))),...
                                          'entropy', struct('done',false, ...
                                                            'subint',struct('name',[],...
                                                                            'signal',struct('name',[],...
                                                                            'format',[false,false,false]))),...
                                          'correlation',struct('done', false,...
                                                               'subint', struct('name',[],...
                                                                                'signal', struct('name', [],...
                                                                                                 'format', [false,false,false]))),...
                                          'stepwise', struct('done', false,...
                                                             'subint', struct('name', [],...
                                                                              'format', [false,false,false])),...
                                          'annotate', struct('done', false,...
                                                             'subint', struct('name', [],...
                                                                              'format',[false,false,false])),...
                                          'transduction', struct('done', false,...
                                                                 'subint', struct('name', [],...
                                                                                  'format',[false,false,false])));
            ...
            for k = 1 : length(figs(i).ints(j).tmp)
                ...
                if strfind(figs(i).ints(j).tmp{k}, 'spike_rates_')
                    figs(i).ints(j).anas.spikerate.done = true;
                    figs(i).ints(j).tmp{k}(1:17) = [];
                    pnt_idx = find(figs(i).ints(j).tmp{k} == '.',1,'last');
                    tmp_unit = figs(i).ints(j).tmp{k}(1:pnt_idx -1);
                    tmp_form = figs(i).ints(j).tmp{k}(pnt_idx +1 : end);
                    switch tmp_form
                        case 'fig'
                            m = 1;
                    ...
                    if isempty(figs(i).ints(j).anas.spikerate.unit(1).name)
                        figs(i).ints(j).anas.spikerate.unit(1).name = tmp_unit;
                        n = 1;
                    else
                        n = [];
                        for o = 1 :length(figs(i).ints(j).anas.spikerate.unit)
                            if strcmp(figs(i).ints(j).anas.spikerate.unit(o).name,tmp_unit)
                                n = o;
                            end
                        end
                        if isempty(n)
                            figs(i).ints(j).anas.spikerate.unit(end+1).name = tmp_unit;
                            n = length(figs(i).ints(j).anas.spikerate.unit);
                            figs(i).ints(j).anas.spikerate.unit(n).format = [false,false,false];
                        end
                    end
                    
                    figs(i).ints(j).anas.spikerate.unit(n).format(m) = true;
                end
