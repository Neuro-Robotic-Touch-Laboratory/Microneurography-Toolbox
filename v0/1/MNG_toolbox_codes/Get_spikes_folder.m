function Get_spikes_folder(input, varargin)
...
function get_spikes_single(filename, par_input)
    ...
    folder = filename(1:end-8);
    if current_par.cont_segment && data_handler.with_raw
        [psegment, sr_psegment] = data_handler.get_signal_sample();
        try
			save([folder data_handler.nick_name '_spikes'], 'spikes', 'index', 'par','psegment','sr_psegment','threshold')
		catch
			save([folder data_handler.nick_name '_spikes'], 'spikes', 'index', 'par','psegment','sr_psegment','threshold','-v7.3')
		end
    else
		try
			save([folder data_handler.nick_name '_spikes'], 'spikes', 'index', 'par')
		catch
			save([folder data_handler.nick_name '_spikes'], 'spikes', 'index', 'par','-v7.3')
		end
    end
