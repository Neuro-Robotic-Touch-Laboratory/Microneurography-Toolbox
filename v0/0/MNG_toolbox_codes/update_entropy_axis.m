...
%         xlabel( cfg.units );
%         title( ['Original time series ' int_name] );
...
if ~isempty(app.entropy_res)
    ...
    %         title('Values of permutation entropy' )
            ...
            app.lbl_entropy_unit4.Visible = 'off';
        case 'PEeq'
            plot(app.ax_entropy2, data_int( end - length( app.entropy_res(int_idx).outdata) + 1:end, 2 ), ...
                  app.entropy_res(int_idx).outdata, 'k', 'LineWidth', .2 ); 
            app.lbl_entropy2.Text = ['Values of permutation entropy for ordinal patterns with tied ranks ' int_name];
    %         xlabel( cfg.units )
    %         title( 'Values of permutation entropy for ordinal patterns with tied ranks' )
            app.ax_entropy3.Visible = 'off';
            app.ax_entropy4.Visible = 'off';
            app.lbl_entropy3.Visible = 'off';
            app.lbl_entropy4.Visible = 'off';
    ...
    %         xlabel( cfg.units )
    %         title( 'Values of ordinary pattern distribution permutation entropy' )
            app.ax_entropy3.Visible = 'off';
            app.ax_entropy4.Visible = 'off';
            app.lbl_entropy3.Visible = 'off';
            app.lbl_entropy4.Visible = 'off';
    ...
    %         title( 'Values of robust permutation entropy' )
            ...
            app.lbl_entropy_unit3.Visible = 'on';
            app.lbl_entropy_unit4.Visible = 'on';
    end
end
end
