...
%     SimpleSpikesSorting = app.chkbx_spike_sorting.Value;
    ...
    spikes=load([writable_folder '\temp_spikes.mat'],'spikes','index','threshold');% spikes=load('temp_spikes.mat','spikes','index','threshold');
    ...
    if app.chkbx_sorting_trans.Value
        ...
        [spike_res.spikes, spike_res.spike_idx, spike_res.spike_ts,spike_res.cluster,spike_res.extremes(:,2), spike_res.extremes(:,1), spike_res.extremes(:,3)] ...
            =comBIN12_v03(spikes,[writable_folder '\temp.mat'],app.edt_trans_dip1.Value,app.edt_trans_peak.Value,app.edt_trans_dip2.Value);
%         [spike_res.spikes, spike_res.spike_idx, spike_res.spike_ts,spike_res.cluster,spike_res.extremes(:,2), spike_res.extremes(:,1), spike_res.extremes(:,3)] ...
%             =comBIN12_v03(spikes,'temp',app.edt_trans_dip1.Value,app.edt_trans_peak.Value,app.edt_trans_dip2.Value);
        %%
    else
        [spike_res.spikes, spike_res.spike_idx, spike_res.spike_ts,spike_res.cluster,spike_res.extremes(:,2), spike_res.extremes(:,1), spike_res.extremes(:,3)] ...
...
%             =comBIN12_v03(spikes,'temp',1,1,1);
    ...
    spike_res = sel_spikes(spike_res, ts, app.burst_ints); 
    clear data ts_msna
end
%%
