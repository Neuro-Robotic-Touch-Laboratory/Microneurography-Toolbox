...
function [rpc, fig, stats] = BlandAltman_ax(ax1,ax2,varargin)

[fig, data, params] = ParseInputArguments(varargin{:});
[cAH, baAH, fig, drawAreaPos] = ConfigAxes(ax1,ax2,fig, params.processCorrelationOnly); %% initialize figures

% Correlation plot
stats = CalcCorrelationStats(data, params);
PlotCorrelation(cAH, data, params);                         %%plot datapoints
params = FormatPlotAxes(cAH, data, params);
DisplayCorrelationStats(cAH, params, stats);                %% plot correlation lines
...
function [cAH, baAH, fig, drawAreaPos] = ConfigAxes(ax1,ax2,fig, processCorrelationOnly)
